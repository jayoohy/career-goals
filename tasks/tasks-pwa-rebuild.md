...
- [ ] 10.0 Data safety + logging discipline (feedback round 5: everything was loggable at once; data vanished between sessions)
  ...
  - [x] 10.5 Fixed the real cause of the "green splash on almost every navigation" report: `public/sw.js` cache-first'd everything (including JS chunks/route data) into one never-versioned cache, so after a few deploys the phone served a mix of build artifacts and Next forced a full-page reload on the mismatch — every reload re-ran the whole boot sequence. Rewrote it: `/api/*` bypassed entirely, navigations/data network-first, only content-hashed `/_next/static/*` cache-first, cache bumped to v2 so the poisoned v1 cache is purged on update. Also stopped `AppInit` from blocking first render on the sync round-trip when local data already exists (sync runs in the background and only reloads once if it actually replaces local data); added a 6s timeout + `no-store` to the sync fetches so a slow/hung request can no longer hold up launch.
